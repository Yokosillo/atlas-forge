...
def _no_real_runtime(monkeypatch):
    ...
    T-AF004-US01-02/US02-01, extendido a Codex en T-AF024-US11-13."""
    import atlas_forge.runtime.claude_code as claude_code_module
    import atlas_forge.runtime.codex as codex_module
...
def test_launch_arquitecto_on_claude_code_and_developer_on_opencode_with_model(
    ...
    # Arquitecto (claude-code, persistente): mecanismo tmux clásico, vivo.
    assert is_runtime_alive(arquitecto_instance, socket_name=isolated_socket) is True
    # Developer (opencode): mecanismo tmux clásico, vivo (T-AF023-US04
    # revertida; ambos runtimes se lanzan por tmux).
...
def test_launch_developer_on_codex_with_model(
    ...
    """T-AF024-US11-13 (2026-08-17, ampliación de alcance explícita del
...
def test_launching_developer_twice_creates_two_distinct_instances(
    ...
    """T-AF005-US01-04: `launch_agent` con `DEVELOPER_ROLE` ya no
...
def test_launching_arquitecto_twice_still_reuses_the_existing_agent(
    ...
    igual que Critic lo hacía antes de eliminarse (AF-022)."""
    session = _active_session()
...
def test_unrecognized_runtime_is_rejected(tmp_path) -> None:
    ...
    T-AF024-US11-13 (2026-08-17) lo activó como runtime real."""
    session = _active_session()
