"""Tests para el módulo de informes de cierre de Job (T-FB022-US06-03,
T-FB022-US06-04)."""
...
def test_write_job_report_creates_file_in_story_dir(tmp_path) -> None:
    job = Job(
        ...
        story_id="US-FB022-06",
    )

    path = write_job_report(job, reports_root=tmp_path)

    assert path.exists()
    assert path.parent.name == "US-FB022-06"
    content = path.read_text()
    assert "US-FB022-06" in content
...
def test_write_job_report_creates_story_dir_if_missing(tmp_path) -> None:
    job = Job(
        ...
        status="completed",
        result="ok",
        story_id="US-FB999-99",
    )

    path = write_job_report(job, reports_root=tmp_path)

    assert path.parent.exists()
    assert path.parent.name == "US-FB999-99"
...
def test_write_job_report_does_not_overwrite_different_job(tmp_path) -> None:
    ...
    content_a = (tmp_path / "US-FB022-06" / "job-a.md").read_text()
    content_b = (tmp_path / "US-FB022-06" / "job-b.md").read_text()
...
def test_read_job_report_returns_none_for_missing_file(tmp_path) -> None:
    result = read_job_report("US-FB999-99", "nonexistent", reports_root=tmp_path)
...
def test_read_job_report_returns_content_for_existing_file(tmp_path) -> None:
    ...
    content = read_job_report("US-FB022-06", "job-read-test", reports_root=tmp_path)
...
def test_concurrent_writes_do_not_collide(tmp_path) -> None:
    """T-FB022-US06-04: dos Developer completándose en ventana solapada no
    ...
    for i, job in enumerate(jobs):
        report_path = tmp_path / "US-FB022-06" / f"{job.id}.md"
