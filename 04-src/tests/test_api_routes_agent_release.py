...
from atlas_forge.agents import mark_unavailable, mark_working
...
def test_post_agent_release_removes_an_unavailable_developer(
    tmp_path: Path, isolated_socket: str, monkeypatch
) -> None:
    """T-AF005-US01-09 criterios 2 y 5: POST /agents/{id}/release sobre un
    Developer `unavailable` lo retira de la sesión y libera su plaza —
    GET /agents deja de listarlo inmediatamente (mismo comportamiento que
    "Detener" para Developer)."""
    ...
    # Simula la caída fuera de atlas_forge: el runtime muere y el estado pasa a
    # `unavailable` (lo que `refresh_agent_liveness` haría al consultar).
    mark_unavailable(agent)
    assert is_runtime_alive(runtime_instance, socket_name=isolated_socket) is True

    client = TestClient(create_app())

    response = client.post(f"/agents/{agent.id}/release")
...
def test_post_agent_release_rejects_a_working_developer(
    tmp_path: Path, isolated_socket: str, monkeypatch
) -> None:
    """Criterio 2: liberar un `working` devuelve 400 — a un agente vivo no
    se le libera, se le detiene."""
    _project, session = _active_project_and_session(tmp_path, monkeypatch)
    ...
    # El agente sigue en la sesión y listado en GET /agents.
    assert client.get("/agents").json()[0]["id"] == agent.id


def test_post_agent_release_returns_404_for_unknown_agent_id(
    tmp_path: Path, monkeypatch
) -> None:
    _active_project_and_session(tmp_path, monkeypatch)
    client = TestClient(create_app())

    response = client.post("/agents/does-not-exist/release")

    assert response.status_code == 404


def test_post_agent_release_returns_404_when_no_session_is_active() -> None:
    client = TestClient(create_app())

    response = client.post("/agents/whatever/release")
