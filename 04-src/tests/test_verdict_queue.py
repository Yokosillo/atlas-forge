...
(T-FB022-US07-01 / T-FB022-US07-02)."""
...
def test_verdict_queue_fifo_order_and_no_overlap() -> None:
    """T-FB022-US07-02: fuerza dos finalizaciones casi simultáneas de
    ...
    with patch(
        ...
        enqueue_architect_verdict("US-FB022-07-A", None, "default")
        enqueue_architect_verdict("US-FB022-07-B", None, "default")

        time.sleep(0.2)

        status = get_verdict_queue_status()
        assert status["active"] == "US-FB022-07-A", (
            f"Esperado active='US-FB022-07-A', obtenido {status}"
        )
        assert "US-FB022-07-B" in status["waiting"], (
            f"Esperado 'US-FB022-07-B' en waiting, obtenido {status}"
        )
        ...
        status = get_verdict_queue_status()
        assert status["active"] == "US-FB022-07-B", (
            f"Esperado active='US-FB022-07-B' tras liberar el primero, "
            f"obtenido {status}"
        )
...
def test_do_dispatch_verdict_does_not_find_agent_by_old_critic_role() -> None:
    ...
    `ARQUITECTO_ROLE` es válido tras el rename de T-FB022-US01-03."""
    session = DevelopmentSession(id="s1", project_id="p1")
    activate(session)
    old_critic = Agent(
...
def test_verdict_queue_enqueue_does_not_block() -> None:
    ...
    with patch(
        ...
        enqueue_architect_verdict("US-FB022-07-C", None, "default")
        time.sleep(0.1)

        start = time.monotonic()
        enqueue_architect_verdict("US-FB022-07-D", None, "default")
        ...
        status = get_verdict_queue_status()
        assert status["active"] == "US-FB022-07-C"
        assert "US-FB022-07-D" in status["waiting"]
