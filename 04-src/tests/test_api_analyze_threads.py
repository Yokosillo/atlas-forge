"""Tests de `POST /backlog/epic/{epic_id}/analyze-threads` (T-AF026-US04-01,
corrección T-AF026-US02-02B): endpoint determinista de análisis de hilos de
...
def _seed_backlog_with_two_threads(project_path: Path) -> None:
    ...
    _write_epic(backlog / "epics" / "AF-900-test.md", "AF-900")
    _write_task(backlog / "tasks" / "T-AF900-US01-01.md", "T-AF900-US01-01", epic="AF-900")
    _write_task(backlog / "tasks" / "T-AF900-US02-01.md", "T-AF900-US02-01", epic="AF-900")
...
def test_analyze_threads_returns_404_for_unknown_epic(tmp_path: Path, monkeypatch) -> None:
    ...
    response = client.post("/backlog/epic/AF-999/analyze-threads")
...
def test_analyze_threads_uses_default_num_agents_when_not_specified(
    ...
    response = client.post("/backlog/epic/AF-900/analyze-threads")
...
def test_analyze_threads_accepts_configurable_num_agents(
    ...
    """Criterio de aceptación (corrección T-AF026-US02-02B): N es un
    parámetro de entrada configurable, nunca fijo a 2."""
    project_path = _active_project(tmp_path, monkeypatch)
    _seed_backlog_with_two_threads(project_path)
    client = TestClient(create_app())

    response = client.post("/backlog/epic/AF-900/analyze-threads?num_agents=5")
...
def test_analyze_threads_rejects_zero_agents(tmp_path: Path, monkeypatch) -> None:
    ...
    response = client.post("/backlog/epic/AF-900/analyze-threads?num_agents=0")
