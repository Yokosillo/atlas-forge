"""Tests de FB-026: grafo de dependencias, nivel topologico, hilos y
reparto (US-FB026-01 a US-FB026-03).

El caso real de FB-022 (34 Tasks, 4 hilos, con el cruce Tester->paralelismo
...
def _fb022_fixture(tmp_path: Path) -> tuple[Path, list[str]]:
    """Backlog sintetico que reproduce la estructura de dependencias real de
    FB-022: 34 Tasks, 4 hilos tematicos, con los cruces detectados
    ...
    Hilos esperados:
      ...
      hilo-4: US12 (Tester) — cruza con hilo-3 via T-FB022-US12-01 -> US-FB022-06
    """
...
class TestBuildTaskGraph:
    def test_builds_graph_with_correct_n_nodes(self, tmp_path: Path):
        ...
        assert task_graph.epic == "FB-022"

    def test_task_with_no_deps_has_empty_edges(self, tmp_path: Path):
        backlog_path, _ = _fb022_fixture(tmp_path)
        graph = load_backlog(backlog_path)
        task_graph = build_task_graph(graph, "FB-022")
        assert task_graph.edges["T-FB022-US01-01"] == ()
        assert task_graph.edges["T-FB022-US03A-01"] == ()

    def test_task_with_explicit_task_dep_has_edge(self, tmp_path: Path):
        backlog_path, _ = _fb022_fixture(tmp_path)
        graph = load_backlog(backlog_path)
        task_graph = build_task_graph(graph, "FB-022")
        assert "T-FB022-US01-01" in task_graph.edges["T-FB022-US01-02"]

    def test_us_dependency_resolves_to_all_tasks(self, tmp_path: Path):
        """US-FB022-06 -> T-FB022-US06-01/02/03/04 via _resolve_us_deps."""
        backlog_path, _ = _fb022_fixture(tmp_path)
        graph = load_backlog(backlog_path)
        task_graph = build_task_graph(graph, "FB-022")

        t12_01_deps = task_graph.edges.get("T-FB022-US12-01", ())
        for tid in ("T-FB022-US06-01", "T-FB022-US06-02",
                     "T-FB022-US06-03", "T-FB022-US06-04"):
            assert tid in t12_01_deps, (
                f"T-FB022-US12-01 should depend on {tid} (US-FB022-06 resolved)"
            )

    def test_missing_refs_reported(self, tmp_path: Path):
        """FB-022 fixture no tiene referencias rotas (todas las US existen)."""
        backlog_path, _ = _fb022_fixture(tmp_path)
        graph = load_backlog(backlog_path)
        task_graph = build_task_graph(graph, "FB-022")
    ...
    def test_missing_ref_reported_for_nonexistent_task(self, tmp_path: Path):
        ...
        backlog = tmp_path / "backlog"
        _make_task_for = lambda tid, us, deps: _make_task(tid, us, deps, epic="FB-099")
        _make_us_for = lambda us: _make_us(us, epic="FB-099")
        ...
        task_graph = build_task_graph(graph, "FB-099")
        assert "T-FB099-US99-99" in task_graph.missing_refs
...
class TestTopologicalLevels:
    def test_root_tasks_have_level_zero(self, tmp_path: Path):
        ...
        assert levels["T-FB022-US01-01"] == 0
        assert levels["T-FB022-US03A-01"] == 0
        assert levels["T-FB022-US06-01"] == 0
        assert levels["T-FB022-US09-01"] == 0
    ...
    def test_direct_dep_increases_level(self, tmp_path: Path):
        ...
        assert levels["T-FB022-US01-02"] >= 1
    ...
    def test_chain_increases_progressively(self, tmp_path: Path):
        ...
        assert levels["T-FB022-US06-01"] == 0
        assert levels["T-FB022-US06-02"] >= 1
        assert levels["T-FB022-US06-03"] >= 2
        assert levels["T-FB022-US06-04"] >= 3
...
class TestGroupThreads:
    def test_produces_4_threads_for_fb022(self, tmp_path: Path):
        """Caso real de FB-022: 34 Tasks agrupadas en 4 hilos."""
        backlog_path, _ = _fb022_fixture(tmp_path)
        graph = load_backlog(backlog_path)
        task_graph = build_task_graph(graph, "FB-022")
    ...
    def test_detects_cross_between_threads(self, tmp_path: Path):
        ...
        assert ("T-FB022-US02-02", "T-FB022-US03A-01") in cross_pairs, (
    ...
    def test_independent_tasks_go_to_different_threads(self, tmp_path: Path):
        ...
        _write(backlog, "user-stories", "US-FB099-02-test.md",
               _make_us("US-FB099-02", epic="FB-099"))
...
class TestReparto:
    def test_reparto_with_one_agent_assigns_all_to_same(self, tmp_path: Path):
        backlog_path, _ = _fb022_fixture(tmp_path)
        graph = load_backlog(backlog_path)
        analysis = analyze_epic_threads(graph, "FB-022")
...
class TestAnalyzeEpicThreads:
    ...
    def test_deterministic_across_calls(self, tmp_path: Path):
        backlog_path, _ = _fb022_fixture(tmp_path)
        graph = load_backlog(backlog_path)
        a1 = analyze_epic_threads(graph, "FB-022")
        a2 = analyze_epic_threads(graph, "FB-022")
    ...
    def test_cycle_in_epic_propagates(self, tmp_path: Path):
        ...
        with pytest.raises(ValueError, match="Ciclo detectado"):
            analyze_epic_threads(graph, "FB-099")
...
class TestPersistAnalysis:
    def test_format_produces_markdown_with_all_sections(self, tmp_path: Path):
        from brain.backlog.dependency_graph import format_thread_analysis_markdown
    ...
    def test_persist_creates_file_with_unique_path(self, tmp_path: Path):
        from brain.backlog.dependency_graph import persist_thread_analysis
    ...
    def test_persist_two_executions_no_overwrite(self, tmp_path: Path):
        ...
        files = list((reports_root / "FB-022").glob("analisis-hilos-*.md"))
        assert len(files) >= 2
