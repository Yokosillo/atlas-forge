...
# Servidor tmux por defecto de Atlas Forge. Un socket_name propio evita
...
DEFAULT_SOCKET_NAME = "atlas-forge"
...
def wait_for_shell_prompt(
    ...
    """Espera a que el pane de `session_name` muestre un prompt de shell
    (línea que termina en `$`/`#`/`%`), indicador de que la shell está lista
    para recibir input. `create_session` lanza la shell de forma asíncrona;
    enviar `send_keys` antes de que el pane esté listo puede perder el
    comando (carrera observada en el lanzamiento híbrido del Developer).
    Devuelve True si se confirmó el prompt, False si se agotó el tiempo
    (mejor esfuerzo: el caller puede enviar el comando igualmente)."""
    server = _get_server(socket_name)
    deadline = time.monotonic() + timeout_seconds
    while time.monotonic() < deadline:
        session = server.sessions.get(session_name=session_name)
        if session is not None:
            lines = session.active_pane.capture_pane()
            if lines and any(
...
def list_sessions(socket_name: str = DEFAULT_SOCKET_NAME) -> list[str]:
    ...
    AF-031, base para que Atlas Forge pueda reconciliar su registro en memoria
...
def is_alive(session_name: str, socket_name: str = DEFAULT_SOCKET_NAME) -> bool:
    ...
    v1 acordado en AF-004: si la sesión tmux existe, el comando lanzado
