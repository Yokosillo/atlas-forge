...
(T-AF004-US05-01), y lectura bajo demanda del modelo activo de un agente
Claude Code (T-AF024-US11-05).
...
Code enviando `/status` al pane (T-AF024-US11-05) — a diferencia de
...
criterio ya fijado para `set_active_model`/criterio 9 de `US-AF024-11`).
...
Verificado en vivo (`00-gobierno`, T-AF024-US11-05, captura real contra
...
def get_available_model_entries(catalog_path: Path | None = None) -> list[dict]:
    ...
    de configuracion (`models.yml`, T-AF022-US09). Consultable sin necesidad
...
_CATALOG_RUNTIME_TO_REAL_TYPE = {
    ...
    # (kebab-case, T-AF002-US01-01) — unica discrepancia entre ambos
...
def get_active_model(
    ...
    Usa contrato de capacidades (T-AF005-US07-01) para verificar que el
...
def get_active_model_claude_code(
    ...
    pane (T-AF024-US11-05) — INTERACCIÓN ACTIVA, a diferencia de
    ...
    `atlas_forge.models`, una capa por encima de este módulo, que solo conoce
...
def set_active_model_claude_code(
    ...
    *,
    ...
    (T-AF024-US11-13, decisión de producto 2026-08-17) enviando el
...
def set_active_model(
    ...
    *,
    ...
    ## Flujo corregido (T-AF024-US11-13, 2026-08-17) — DOS bugs reales
    ...
    **Bug 2 — cálculo de navegación mezclaba índice de línea de texto con
    ...
    (sigue `idle`/`working` — aunque T-AF024-US11-11 restringe esto a
    ...
    # Bug real corregido (T-AF024-US11-13, 2026-08-17): un fallo a mitad
...
def _find_model_offset_in_filtered_list(lines: list[str], model_name: str) -> int | None:
    ...
    selector de OpenCode (T-AF024-US11-13, decisión de diseño 2026-08-17:
...
def _normalize_model_text(text: str) -> str:
    ...
    minúsculas — bug real corregido (T-AF024-US11-13, 2026-08-17): la
