...
(T-AF016-US01-05): `WS /ws/jobs` (transiciones de Job) y `WS /ws/plans`
(plan del Critic y su progreso de despacho).

## Por qué un hub aparte, no tocar `dispatch_job`/`dispatch_plan`

`dispatch_job` (AF-008 v1) y `dispatch_plan` (US-AF008-04) son funciones
...
transporte de una Epic distinta (AF-016), justo lo que
`02-backlog/epics/AF-016-api-backend.md` pide evitar ("no se duplica
...
estado". `atlas-forge-api` y `atlas_forge` (la TUI) son dos procesos Python
independientes en v1 (ver `02-backlog/epics/AF-016-api-backend.md`,
...
despacho que ocurra DENTRO del proceso `atlas-forge-api` (hoy, la única vía de
...
contradice el alcance ya fijado de AF-016; lo respeta.
