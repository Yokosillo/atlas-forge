"""App FastAPI de Atlas Forge (T-AF016-US01-01, US-AF016-01): esqueleto
...
(ver `atlas_forge.core.session_registry` / `atlas_forge.runtime.agent_runtime_registry`),
...
## Dependencia `websockets` (T-AF016-US01-05)
...
servidor real arrancado (`atlas-forge-api`) con un cliente WebSocket real —no
...
from atlas_forge.api import routes as routes_module
from atlas_forge.api.cache_bust import get_cache_bust_version
from atlas_forge.api.events import jobs_hub, plans_hub, register_event_loops
...
from atlas_forge.tmux import capture_pane_lines, list_sessions
from atlas_forge.workspace.active_project import get_active_project

# T-AF021-US01-01: directorio raíz de la interfaz web estática (`10-web/`).
...
async def _lifespan(app: FastAPI):
    ...
    # (ver `atlas_forge.api.events`). Mecanismo `lifespan` (no `@app.on_event`,
    # deprecado en esta versión de FastAPI).
    register_event_loops(asyncio.get_running_loop())

    # T-AF016-US01-10: arranca la sesión de desarrollo una vez al iniciar
    # el proceso, mismo mecanismo que ya usa `AtlasForgeApp.on_mount`
    ...
    # T-AF016-US01-11, o la selección manual que use `select_active_project`).
    #
    # T-AF016-US01-11: si ya existe una sesión activa al llegar aquí (el
    ...
    #
    # `workspace_root`/`state_dir` (T-AF016-US01-11): se leen de
    ...
    # ubicación por defecto de `atlas_forge.storage`), pero en tests
    ...
    # T-AF031-US02-02: tras resolver la sesión del proyecto activo,
    # reengancha cada sesión tmux real del socket que ya pertenezca a ese
    # proyecto (AF-031, ver `core/session_reconciliation.py`) — sin esto,
    ...
    # flujo de selección de proyecto (AF-001) debe completarse primero.
    #
    ...
    # `dispatch_queue_worker.py` para el porqué:
    ...
    # que esto haga fallar el arranque de `atlas-forge-api` (mismo criterio que
    ...
    # T-AF024-US21-01: arranca el watcher de límite de sesión de Claude
    ...
    # T-AF023-US05-01: arranca el watcher de agentes persistentes — mismo
    # patrón y mismo requisito (`current_session` real ya resuelto) que el
    # watcher de límite de sesión: comprueba periódicamente la
    # operatividad real de los agentes persistentes (Arquitecto, ...) sin
    # depender de `GET /agents`. No relanza (US-AF023-02), solo detecta.
    if current_session is not None:
        routes_module._persistent_agent_watcher = PersistentAgentWatcher(
            current_session,
            socket_name=routes_module._SOCKET_NAME,
        )
...
def create_app() -> FastAPI:
    app = FastAPI(title="Atlas Forge API", lifespan=_lifespan)
    app.include_router(router)

    # T-AF021-US01-01: servir la interfaz web estática bajo un PREFIJO
    ...
    # Fallback SPA (T-AF024): rutas propias por sección (`/ui/roles`,
    ...
    class _SPAStaticFiles(StaticFiles):
        async def get_response(self, path, scope):
            try:
                ...
                # T-AF021-US01-03: inyectar cache-bust version en HTML servido.
    ...
    async def ws_plans(websocket: WebSocket) -> None:
        ...
        despacho (US-AF008-04: construcción, aprobación/rechazo, y cada
    ...
    async def ws_agent_pane(websocket: WebSocket, agent_id: str) -> None:
        ...
        (T-AF032-US01-01) — equivalente en vivo de `GET /agents/{id}/pane`,
        sin que el cliente tenga que pedirlo en bucle.

        Canal 1:1, NO un `_ChannelHub` compartido (`atlas_forge.api.events`):
