"""Rutas HTTP de la API (T-AF016-US01-02 en adelante): cada endpoint es
una envoltura fina sobre una función de dominio ya existente, sin
reimplementar su validación — mismo principio ya fijado en
`02-backlog/epics/AF-016-api-backend.md` ("cada endpoint es una envoltura
...
from atlas_forge.agents.supervision import refresh_agent_supervision
...
from atlas_forge.api.events import jobs_hub, plans_hub
from atlas_forge.api.plan_registry import get_plan, get_plan_lock, list_plans, register_plan
from atlas_forge.backlog.detail import build_epic_detail, build_item_detail, is_epic_item_id
...
from atlas_forge.architect.review_user_story import review_user_story_for_gaps
from atlas_forge.architect.epic_landing import can_approve_landing as can_approve_epic_landing
...
from atlas_forge.dispatcher.job_history_registry import list_jobs_for_session
...
from atlas_forge.dispatcher.job_plan_lifecycle import InvalidJobPlanTransitionError
from atlas_forge.models import Agent, Job
from atlas_forge.models.backlog import ITEM_KIND_TASK, ITEM_KIND_USER_STORY
from atlas_forge.runtime.agent_runtime_registry import get_runtime_instance_for_agent
from atlas_forge.runtime.generic import extract_model_from_runtime, is_runtime_alive
...
from atlas_forge.tmux.manager import DEFAULT_SOCKET_NAME, run_command
from atlas_forge.tmux import capture_pane_lines
from atlas_forge.workspace.active_project import (
    ProjectNotDiscoveredError,
    get_active_project,
    select_active_project,
)
from atlas_forge.workspace.discovery import (
    discover_projects,
...
from atlas_forge.local_tools import ScribeUnavailableError, resumir_estado_backlog
...
# T-AF018-US02-04: la síntesis en prosa (T-AF018-US02-03) es una capa
...
# Mismo patrón que `_SOCKET_NAME` (T-AF016-US01-11): `None` en producción
# resuelve al directorio de trabajo real del proceso / ubicación por
# defecto de `atlas_forge.storage` (igual que `resolve_startup_session()` sin
# argumentos en `_lifespan`, `atlas_forge/api/app.py`) — expuestos como
...
# T-AF008-US10-02: instancia del Dispatcher de fondo de la cola de
# despacho, arrancada en `_lifespan` (`atlas_forge/api/app.py`) — variable de
...
# T-AF024-US21-01: instancia del watcher de límite de sesión de Claude
# Code, arrancada en `_lifespan` — mismo patrón que `_dispatch_queue_worker`
# justo arriba.
_session_limit_watcher = None

# T-AF023-US05-01: instancia del watcher de agentes persistentes
# (Arquitecto y otros con `persistent=true`), arrancada en `_lifespan` —
...
class LaunchAgentRequest(BaseModel):
    ...
    # T-AF005-US01-08 (2026-08-18): número de "slot" de Developer
    ...
    def resolved_runtime_type(self) -> str | None:
        ...
        T-AF005-US07-02: el runtime es ahora la elección de primera clase
        del contrato (`atlas_forge.runtime_model_contract`). Si el request trae
        ...
        ignorar un runtime seleccionado (raíz del bug T-AF024-US12-03).
        ...
        if model:
            from atlas_forge.agent_model import resolve_runtime_for_model
...
class CreateJobRequest(BaseModel):
    ...
    # T-AF024-US15-01: Story a la que pertenece este Job suelto (sin pasar
...
class CreateEpicRequest(BaseModel):
    # T-AF036-US02-01: campos sueltos del formulario "+ Nueva Epic"
    # (T-AF036-US02-04, todavía sin implementar) — `id` se valida en
    # servidor contra `EPIC_ID_PATTERN` (`atlas_forge.backlog.create`), nunca
    # solo en cliente. T-AF036-US18-01: la Epic ya NO tiene `fase` (se
    # versiona); el campo se retira del request.
    id: str
    title: str
    objetivo: str
...
class CreateTaskRequest(BaseModel):
    # T-AF036-US02-03: campos sueltos del formulario "+ Nueva Task"
    # (T-AF036-US02-06, todavía sin implementar) — deliberadamente SIN
...
class SelectProjectRequest(BaseModel):
    ...
    # `atlas_forge/workspace/discovery.py`) — un único campo cubre ambos nombres
...
class RunScriptRequest(BaseModel):
    # T-AF018-US01-03: `message` es el único parámetro que necesita algún
...
def get_project() -> dict:
    """Proyecto activo (`atlas_forge.workspace.active_project`, AF-001) — 404
    explícito si no hay ninguno seleccionado todavía.

    Usa `_STATE_DIR` (T-AF016-US01-11), no el valor por defecto de
    ...
    (`~/.local/share/atlas_forge/`), inconsistencia que se hizo visible al
...
def get_projects() -> list[dict]:
    """Repositorios Git descubiertos en el workspace (T-AF016-US01-11),
    candidatos a proyecto activo — mismo `discover_projects` (AF-001) que
    ya usa `WorkspaceScreen` de la TUI, sobre el mismo `workspace_root`
    por defecto (directorio de trabajo del proceso) que usa
    `resolve_startup_session` en `_lifespan` (`atlas_forge/api/app.py`), para
...
def post_project(body: SelectProjectRequest) -> dict:
    ...
    (`select_active_project`, AF-001) y le da el foco de sesión de
    desarrollo del proceso (T-AF029-US01-02, `focus_project_session`):
    ...
    ## Qué pasa con los agentes de la sesión anterior (AF-029)
    ...
    `atlas_forge.core.session_registry`) y vuelven a ser alcanzables por
    ...
    registro multi-sesión de AF-029 cada proyecto tiene su propia sesión
    ...
    `02-backlog/epics/AF-016-api-backend.md`, sección "Cambio de
    proyecto activo en caliente", y `02-backlog/epics/AF-029-sesiones-proyecto-simultaneas.md`.
    """
    ...
    # T-AF001-US01-06: al cambiar de proyecto activo se invalidan las cachés
...
def post_project_action(action_id: str) -> dict:
    ...
    - `auditar-ux` (T-AF024-US13-03): despacha un Job a la instancia de UX
    ...
    from atlas_forge.actions.transversal import ACCIONES_DISPONIBLES, dispatch_action
    ...
    try:
        ...
        # Defensa en profundidad (T-AF024-US13-03): `_find_agent_by_role`
...
def _serialize_agent(agent: Agent) -> dict:
    ...
    (T-AF005-US05-01, criterio de aceptación: "consultar el agente
    ...
    (`get_runtime_instance_for_agent`, T-AF002-US03-00) — `None`
    ...
    return {
        ...
        # T-AF023-US01-02: estado de supervisión (vivo/colgado/detenido),
...
def get_agents() -> list[dict]:
    ...
    (reutiliza `list_agents`, AF-005). 404 si no hay sesión activa — sin
    sesión no hay agentes que consultar, mismo criterio que `GET
    /session`.

    Antes de devolver cada agente, `refresh_agent_liveness`
    (T-AF016-US01-07) comprueba de forma perezosa si su runtime real
    ...
    cliente vea `idle`/`working` de un proceso que ya no existe. Además,
    `refresh_agent_supervision` (T-AF023-US01-02) calcula el estado de
    supervisión (vivo/colgado/detenido) sin alterar el estado funcional."""
    session = get_current_session()
    if session is None:
        raise HTTPException(
            status_code=404, detail="No hay ninguna sesión de desarrollo activa."
    ...
    return [
        _serialize_agent(
            refresh_agent_supervision(
                refresh_agent_liveness(agent, socket_name=_SOCKET_NAME),
...
def get_agents_options() -> list[dict]:
    """Catalogo de combinaciones rol/modelo disponibles (T-AF022-US11-01):
    ...
    El filtro que excluia Critic + OpenCode (T-AF016-US01-19) se eliminó
    junto con el rol `critic` (AF-022): ya no existe esa combinación que
    ocultar."""
    return [
        {
...
def _reject_incompatible_launch_model(runtime_type: str, model: str) -> None:
    """Criterio 3 de T-AF005-US07-02: rechaza (400) un modelo catalogado
    que pertenece a OTRO runtime. Se rechaza ANTES de lanzar nada, sin
    efectos parciales.

    Corrección T-AF024-US11-13 (2026-08-17): Claude Code SÍ admite
...
def post_agents(body: LaunchAgentRequest) -> dict:
    ...
    `atlas_forge.agents.launch`, AF-005), sin reimplementar su validación.
    `project_path` se resuelve del proyecto activo (AF-001) en vez de
    ...
    Si `initial_job_description` viene informado (T-AF016-US01-16, expone
    T-AF008-US06-01), el lanzamiento encadena el despacho del Job inicial
    ...
    try:
        ...
        if runtime_type is None:
            # Criterio 1/2 de T-AF005-US07-02: el lanzamiento requiere una
        ...
        if model is not None:
            # Criterio 3 de T-AF005-US07-02: un modelo se acepta solo
...
def post_agent_stop(agent_id: str) -> dict:
    ...
    T-AF016-US01-03): detiene su sesión tmux real. Para la mayoría de
    roles (incluido Arquitecto) lo transiciona a `stopped` — nunca a
    `unavailable` — y permanece consultable en `GET /agents` después. Para
    Developer (T-AF024-US12-02, decisión de producto), en cambio, el
...
def post_agent_release(agent_id: str) -> dict:
    """Libera el agente `agent_id` de la sesión activa (`release_agent`,
    T-AF005-US01-09): lo retira por completo de `session.agents` cuando su
    proceso murió o se detuvo fuera de atlas_forge (`unavailable`/`stopped`) —
    sin intentar matar su runtime (ya no está vivo), liberando de inmediato
    su plaza del límite de Developers simultáneos y dejando de listarlo en
    `GET /agents`.

    404 si no hay sesión activa o si el agente no existe. 400 si el agente
    está activo (`idle`/`working`/`limited`): a un agente vivo no se le
    libera, se le detiene con `POST /agents/{id}/stop`. La respuesta
    devuelve el estado del agente justo tras la acción (con su `status`
    `unavailable`/`stopped`, ya no consultable después)."""
    session = get_current_session()
    if session is None:
        raise HTTPException(
            status_code=404, detail="No hay ninguna sesión de desarrollo activa."
...
def get_agent_pane(agent_id: str) -> dict:
    ...
    activa (T-AF016-US01-12). Reutiliza `capture_pane_lines` sin reimplementar:
...
def get_agent_model(agent_id: str) -> dict:
    ...
    estado de OpenCode via `get_active_model` (T-AF004-US05-01).
...
def get_agent_status_model(agent_id: str) -> dict:
    ...
    `/status` del pane (T-AF024-US11-05, `get_active_model_claude_code`)
...
def put_agent_model(agent_id: str, body: dict) -> dict:
    ...
    - OpenCode: `set_active_model` (T-AF004-US05-01, atajos de teclado).
    - Claude Code (T-AF024-US11-13, decision de producto 2026-08-17):
    ...
    # bug real corregido (2026-08-17, T-AF024-US11-13): el frontend
...
def get_agent_available_models(agent_id: str) -> dict:
    ...
    T-AF024-US11-13: Claude Code cambia de modelo enviando '/model <id>'
    ...
    del catalogo de configuracion (`models.yml`, T-AF022-US09), filtrada
...
def send_agent_keys(agent_id: str, body: dict) -> dict:
    ...
    (T-AF024-US11-13). Se usa para enviar empujones sin crear un Job
    ...
    experimentalmente en `T-AF024-US21-01`
    (`atlas_forge/agents/session_limit_watcher.py`): un `Enter` vacío por sí
...
def get_models_preferences() -> dict:
    ...
    y defaults por rol (T-AF022-US10-01).
...
def put_models_preferences(body: UpdateModelsPreferencesRequest) -> dict:
    """Actualiza las preferencias de modelos (T-AF022-US10-01).
...
def get_system_preferences() -> dict:
    """Preferencias de sistema (US-AF024-12): catálogo abierto de valores
...
# Comando fijo de reinicio (T-AF037-US05-01): nunca se interpola nada del
...
# atlas-forge-api`), acotada a ese único comando para no abrir una
# escalación de privilegios genérica.
_RESTART_COMMAND = ["sudo", "/usr/bin/systemctl", "restart", "atlas-forge-api"]


@router.post("/system/restart", status_code=202)
def post_system_restart() -> dict:
    """Reinicia el servicio `atlas-forge-api` (T-AF037-US05-01).
...
def post_jobs(body: CreateJobRequest) -> dict:
    ...
    US-AF008-01/02), sin reimplementar su validación ni su mecanismo de
    ...
    nuevo Job (`create_job(..., previous_job=...)`, US-AF008-02) — mismo
    mecanismo que `JobsScreen` usa hoy para encadenar Developer → Critic.

    Publica en `WS /ws/jobs` (T-AF016-US01-05) el evento `created` antes
    de despachar, y el evento final (`completed`/`failed`) después —
    `dispatch_job` en sí no conoce a sus observadores (ver
    `atlas_forge.api.events` para la justificación de no tocar el dominio).
    ...
    # T-AF024-US15-01 / T-AF008-US14-02: mismo ciclo de informe de cierre
...
def get_jobs() -> list[dict]:
    ...
    T-AF002-US03-03), no duplicado. 404 si no hay sesión activa."""
    session = get_current_session()
    if session is None:
        raise HTTPException(
            status_code=404, detail="No hay ninguna sesión de desarrollo activa."
...
def post_job_cancel(job_id: str) -> dict:
    """Cancela el Job `job_id` en curso (T-AF016-US01-15, expone
    `request_cancellation`, T-AF008-US05-01): localiza el Job en el
...
def post_plans(body: CreatePlanRequest) -> dict:
    ...
    Story), reutilizando `build_job_plan_for_story` (T-AF008-US04-01) sin
    ...
    US-AF008-04, reconfirmado aquí).

    US-AF008-04 ya existe en este momento de la implementación de
    Atlas Forge, así que este endpoint no devuelve nunca 501 en la
...
def get_plans() -> list[dict]:
    """Lista todos los planes registrados en el proceso (T-AF016-US01-14):
    ...
    primer lugar — ver T-AF016-US01-18) no tenía forma de descubrir qué
...
def _settle_plan_decision(plan_id: str, plan, approved: bool) -> tuple[dict, bool]:
    """Sección crítica de la decisión (T-AF016-US01-08): protegida por el
    ...
    (ver docstring de módulo de `atlas_forge.api.plan_registry`).
...
def post_plan_approve(plan_id: str) -> dict:
    ...
    `atlas_forge/api/events.py`) — solo invoca una función genérica.

    Idempotente (T-AF016-US01-08): dos llamadas casi simultáneas para el
...
def post_plan_reject(plan_id: str) -> dict:
    ...
    T-AF008-US04-02) — no se despacha ningún Job de este plan.

    Idempotente (T-AF016-US01-08), mismo mecanismo que `post_plan_approve`:
...
def post_plan_cancel(plan_id: str) -> dict:
    """Cancela el plan `plan_id` en curso (T-AF016-US01-17, expone
    `job_plan_cancellation.request_cancellation`, T-AF008-US08-01):
    ...
    ## con evidencia, no por simetría automática con T-AF016-US01-15)
...
def get_backlog() -> dict:
    ...
    (T-AF020-US01-01): envoltura fina de `build_backlog_report`
    (T-AF018-US02-02) — mismo dict que ya usa `POST
...
def get_dispatch_queue() -> dict:
    """Estado completo de la cola de despacho (T-AF008-US10-01/-04): qué
    Tasks están `queued`/`dispatched`/`awaiting_tester`/`completed`/
    `failed`, en qué orden por prioridad (`Crítica` > `Alta` > `Media` >
    `Baja`/sin prioridad, reutilizando `priority_rank` de
    `atlas_forge.backlog.report` — mismo criterio de orden que el resto del
    backlog, sin un segundo cálculo paralelo). Base del criterio 6 de
    `US-AF008-10` ("consultable desde la pantalla Backlog sin necesitar
    la pantalla Plan ni Agentes").

    T-AF008-US10-04: el estado MOSTRADO de cada entrada se deriva
    cruzando su estado almacenado con el estado REAL del fichero de la
    Task (la fuente de verdad tras AF-040) — `derive_effective_status`.
    Cada entrada serializada incluye `effective_status`, y los grupos
    devueltos son los derivados: `dispatched` solo contiene Tasks
    realmente `IN_PROGRESS`, `awaiting_tester` las `IN_REVIEW` (el
    Developer que la cerró retenido), `completed` las `DONE`, y una Task
    `DONE`/`READY` nunca aparece como "en curso".
    ...
    def _effective(entry) -> str:
        return derive_effective_status(entry, state_by_task_id.get(entry.task_id))

    # Solo las `queued` se ordenan por prioridad (es el orden que le
    # importa al Dispatcher, `T-AF008-US10-02`, para decidir cuál sacar
    # primero) — las `dispatched`/`failed` ya no compiten por orden de
    # despacho, se listan en su propio orden de inserción (histórico).
    queued = sorted(
        (e for e in entries if _effective(e) == STATUS_QUEUED),
    ...
    def _serialize(entry) -> dict:
        return {
            ...
            "status": entry.status,
            "effective_status": _effective(entry),
            "enqueued_at": entry.enqueued_at,
            "agent_id": entry.agent_id,
            "agent_name": entry.agent_name,
            "result": entry.result,
            "dispatched_at": entry.dispatched_at,
            # T-AF036-US17-01: timestamp de cierre de la entrada
            # (`completed`/`failed`); `None` si aún no ha terminado.
            "finished_at": entry.finished_at,
        }

    return {
        "queued": [_serialize(e) for e in queued],
        "dispatched": [_serialize(e) for e in dispatched],
...
@router.delete("/backlog/queue/history")
def delete_backlog_queue_history() -> dict:
    """Borra el histórico de la cola de despacho (T-AF036-US17-02): elimina
    del `dispatch_queue.json` las entradas terminales (`completed`/`failed`),
    conservando las en curso (`queued`/`dispatched`).

    No toca el estado real de las Tasks ni el Dispatcher — solo el registro
    de auditoría. Devuelve el número de entradas borradas. 404 si no hay
    proyecto activo."""
    project = _active_project_or_404()
...
def post_enqueue_task(task_id: str) -> dict:
    ...
    encolada para desarrollo (T-AF008-US10-01, criterio de aceptación 1
    de `US-AF008-10`) — sin pasar por el flujo de Plan/aprobación.
    ...
    if item.state != "READY":
        raise HTTPException(
            status_code=400,
            detail=f"La Task '{task_id}' no está en estado 'READY' (estado real: '{item.state}') — no se puede encolar.",
        )
    ...
    # T-AF008-US14-01: `TO_DEVELOP` (AF-040; antes EN_DESARROLLO) en el
    # propio `state` del fichero real es ahora la fuente de verdad de
    # "lista para desarrollo" — la entrada en `dispatch_queue.json`
    # (arriba) se conserva como mecanismo auxiliar de orden FIFO/auditoría
    # (`dispatch_reason`, `dispatched_at`), pero ya no es el único lugar
    # donde "encolada" se registra. Se escribe DESPUÉS de la entrada JSON:
    # si esto fallara, la entrada JSON ya escrita seguiría siendo
    # consistente con el estado anterior (`READY`) del fichero, sin dejar
    # un estado a medias peor que el actual.
...
def post_enqueue_all_tasks(us_id: str) -> dict:
    ...
    prefijo del propio `task_id` — confirmado en `T-AF036-US01-04` que
...
def delete_dequeue_task(task_id: str) -> dict:
    ...
    (`T-AF008-US10-02`) la haya tomado (T-AF008-US10-01, criterio de
    aceptación 7 de `US-AF008-10`) — sin ningún efecto secundario, mismo
    ...
    # T-AF008-US14-01: desencolar revierte `state: TO_DEVELOP` a `READY`
    # (AF-040; antes EN_DESARROLLO → TO_DO) en el fichero real — mismo
    # criterio de reversibilidad ya documentado arriba ("sin ningún efecto
    # secundario"), ahora que TO_DEVELOP vive en el propio estado. Mejor
    # esfuerzo: si el item ya no aparece en el grafo (caso borde
    # improbable), no se bloquea la respuesta 200 del desencolado real (ya
    # confirmado arriba) por un fallo de esta segunda escritura.
    graph = _load_active_backlog_graph(project)
    item = graph.items.get(task_id)
    if item is not None and item.state == "TO_DEVELOP":
        set_item_state(item.path, "READY")
...
def _resolve_editable_item(graph, item_id: str):
    ...
    en línea de `US-AF036-08`. Devuelve 404 si no existe como User
    ...
    T-AF036-US08-01, criterio de aceptación 5)."""
...
def put_backlog_item_priority(item_id: str, body: dict) -> dict:
    ...
    (T-AF036-US08-01) desde su línea de título en el listado raíz, sin
    desplegar el detalle — reutiliza `atlas_forge.backlog.edit.set_item_priority`,
...
@router.put("/backlog/{item_id}/fase")
def put_backlog_item_fase(item_id: str, body: dict) -> dict:
    """Cambia el campo `fase` del fichero real de una Epic/User Story
    (T-AF036-US14-01) — reutiliza `atlas_forge.backlog.edit.set_item_fase`, que
    valida el contenido resultante con el validador determinista antes de
    persistir.

    Body: `{"fase": "<string>" | null}`. Conjunto cerrado (T-AF036-US14-05):
    `fase` debe ser una de `Fase 0.9`/`Fase 0.9.1`/`Fase 0.9.2` o `null`
    (sin fase) — cualquier otro valor (incluidos los 0.x legados) responde
    400 listando las fases válidas y NO escribe nada. 400 también si el
    contenido resultante no pasa el validador — `detail` verbatim — y no
    escribe nada; 404 si el item no existe."""
    project = _active_project_or_404()
    graph = _load_active_backlog_graph(project)
    item = graph.items.get(item_id)
    if item is None:
    ...
    new_fase = body.get("fase")
    try:
        set_item_fase(item.path, new_fase)
    except InvalidFieldValueError as error:
        raise HTTPException(status_code=400, detail=str(error)) from error
    except BacklogValidationError as error:
        raise HTTPException(status_code=400, detail=str(error)) from error
...
def put_backlog_item_state(item_id: str, body: dict) -> dict:
    ...
    (T-AF036-US08-01) desde su línea de título, sin desplegar el
    detalle — reutiliza `atlas_forge.backlog.edit.set_item_state`, que valida
    ...
    hijos `DONE` — criterio de aceptación 2 de `US-AF036-08` y de la
    propia Task, reutilizando el mecanismo sin duplicarlo.

    Si el nuevo estado es `TO_DEVELOP` (AF-040; antes `EN_DESARROLLO`) y
    el item es una User Story (T-AF008-US14-04): atajo funcional
    equivalente a pulsar "Marcar toda la Story para desarrollo" — encola
    automáticamente todas sus Tasks `READY` (mismo criterio idempotente
    que `enqueue-all`), coordinado para no divergir entre los dos caminos.

    Body: `{"state": "READY" | "TO_DEVELOP" | "IN_PROGRESS" | "IN_REVIEW" | "DONE"}`.
    400 si el valor no pertenece al conjunto cerrado, o si el contenido
...
def post_backlog_epic(body: CreateEpicRequest) -> dict:
    """Crea una Epic nueva desde cero (T-AF036-US02-01, US-AF036-02):
    ...
    Arquitecto (`atlas_forge.backlog.create.create_epic`) antes de persistir —
    precondición de backend para el formulario "+ Nueva Epic"
    (`T-AF036-US02-04`, todavía sin implementar).
    ...
    400 si `id` no tiene formato `AF-\\d{3,}` (validado en servidor,
    ...
    try:
        path = create_epic(backlog_path, body.id, body.title, body.objetivo)
...
def post_backlog_epic_user_story(epic_id: str, body: CreateUserStoryRequest) -> dict:
    ...
    (T-AF036-US02-02, US-AF036-02): escribe
    `02-backlog/user-stories/{id}-{slug(title)}.md` a partir de campos
    sueltos, pasando por el mismo validador determinista que usa el
    Arquitecto (`atlas_forge.backlog.create.create_user_story`) antes de
    persistir — precondición de backend para el formulario "+ Nueva User
    Story" (`T-AF036-US02-05`, todavía sin implementar).
    ...
    formato `US-AFNNN-nn`, si `priority` no pertenece al conjunto cerrado
...
def post_backlog_us_task(us_id: str, body: CreateTaskRequest) -> dict:
    ...
    (T-AF036-US02-03, US-AF036-02): escribe
    ...
    (`atlas_forge.backlog.create.create_task`) antes de persistir —
    precondición de backend para el formulario "+ Nueva Task"
    (`T-AF036-US02-06`, todavía sin implementar).
    ...
    `user-stories/`. 400 si `id` no tiene formato `T-AFNNN-USnn-mm`, si
    ...
    # T-AF008-US15-01/-02, criterio 5: crear una Task manual sobre una US
    ...
    # `TO_PLAN` (AF-040; antes EN_DISEÑO), esperando al Arquitecto —
    # criterio 9 de la US: "+ Nueva Task" sigue disponible ahí), no se
    # toca. La US pasa al estado derivado `READY` (su Task menos
    # avanzada está `READY`).
    ...
    if us_item_after_create is not None and us_item_after_create.state == "NO_TASKS":
        # T-AF036-US22-01: `NO_TASKS -> READY` (derivación automática al
        # crear la primera Task) no la modela la máquina canónica — es la
        # derivación interna del pipeline, se fuerza.
        set_item_state(us_item_after_create.path, "READY", force=True)
...
def get_backlog_item(item_id: str) -> dict:
    ...
    proyecto activo (T-AF020-US01-01), sin reimplementar el parseo de
    Markdown: reusa `load_backlog` (T-AF018-US02-01) para el grafo ya
    calculado y `atlas_forge.backlog.detail` para extraer las secciones
    ...
    Un `item_id` con forma `AF-xxx` (p. ej. `AF-020`) se resuelve como
...
@router.get("/backlog/epic/{epic_id}/coverage")
def get_epic_coverage(epic_id: str) -> dict:
    """Cobertura del alcance v1 declarado de la Epic `epic_id`
    (T-AF036-US05-01): lee la sección `## Alcance v1 (mínimo)` del fichero
    real de la Epic y la compara contra sus User Stories/Tasks reales con
    un detector determinista-aproximado (`compute_epic_coverage`,
    `atlas_forge.backlog.coverage`), sin reimplementar nada aquí.

    Casos explícitos (criterios de la Task):
    - Epic sin fichero propio en `02-backlog/epics/` -> 404 verbatim.
    - Epic que no declara la sección -> 200 con `declared_alcance: null` y
      el mensaje explícito "Esta Epic no declara un alcance v1 mínimo — no
      se puede calcular cobertura", nunca un resultado vacío ambiguo.
    - Con sección -> 200 con `declared_alcance`, `points`, `gaps` y
      `approximate: true` con aviso de que es detección aproximada."""
    project = get_active_project(state_dir=_STATE_DIR)
    if project is None:
        raise HTTPException(status_code=404, detail="No hay ningún proyecto activo.")
    ...
    if result is None:
        raise HTTPException(
            status_code=404,
            detail=f"No existe ningún fichero de Epic con id '{epic_id}' en el backlog activo.",
        )
    return result
...
def _job_plan_builder_story_id(story_id: str) -> str:
    ...
    `US-AF020-01`) al formato que espera `_pending_task_files_for_story`/
    `build_job_plan_for_story` (`job_plan_builder.py`): SIN el prefijo
    `US-`, con el número de Story pegado tras `US` (`AF020-US01`) — el
    mismo prefijo literal que ya usan los nombres reales de fichero de
    Task en `02-backlog/tasks/` (`T-AF020-US01-01-...md`, NO
    `T-US-AF020-01-...md`). Verificado directamente contra el `02-backlog/`
    real de este proyecto y contra `test_job_plan_builder.py` (que
    construye sus Tasks de prueba con `story_id = "AF999-US01"`, nunca
    `"US-AF999-01"`) — sin esta conversión, `_pending_task_files_for_story`
    ...
    `_mark_story_tasks_done` y `read_acceptance_criteria` (T-AF022-US13-01B):
...
def _build_launch_development_description(story_id: str, story_detail: dict, tasks_dir: Path) -> str:
    ...
    (T-AF020-US02-01): objetivo/historia real de la US (ya resuelto por
    ...
    (`atlas_forge.dispatcher.job_plan_builder`) tal cual — mismo mecanismo ya
...
def post_launch_development(story_id: str, body: LaunchDevelopmentRequest) -> dict:
    ...
    resuelto (T-AF020-US02-01): construye la `description` del Job
    concatenando el objetivo/historia real de la US (mismo contenido que
    `GET /backlog/{item_id}`, T-AF020-US01-01) y los títulos de sus Tasks
    ...
    AF-005/AF-008) — no un despachador paralelo. El Job resultante es
...
# ------------------------------------------------------------------ AF-026
# Análisis de hilos de desarrollo de una Epic (US-AF026-01 a US-AF026-04):
...
def post_analyze_epic_threads(epic_id: str, num_agents: int = 2) -> dict:
    ...
    fijo (corrección 2026-08-06, auditoría de cierre de Fase 0.9: el
    valor estaba hardcodeado sin posibilidad de override, contradiciendo
    el criterio de aceptación de la Epic AF-026).

    No despacha un Job al Arquitecto — el análisis es determinista (mismo
    criterio que `testear` en AF-025), ya implementado en
    `atlas_forge.backlog.dependency_graph`. El informe se persiste en
...
def post_propose_stories(epic_id: str) -> dict:
    ...
    # T-AF036-US03-02 (US-AF036-03): se invoca el módulo de dominio
    # `plan_epic_landing` (T-AF036-US03-01) para el cálculo puro
    # (propuesta + validación determinista + autoauditoría) en un único
    # punto, en vez de recomponer el pipeline aquí (antes se llamaba a
    # `run_us_pipeline`, que reimplementaba esa misma composición y, de
    # paso, la escritura). La conexión no duplica lógica de negocio: la
    # decisión de escribir se delega a `can_approve_epic_landing` y la
    # escritura en sí a `write_approved_stories`.
    plan = plan_epic_landing(epic_context)

    output_dir = str(Path(project.path) / "02-backlog" / "user-stories")
    if can_approve_epic_landing(plan, auto_approve=False) and output_dir:
        write_approved_stories(plan.proposal, output_dir)

    stories_data = []
    for story in plan.proposal.stories:
        stories_data.append({
            "id": story.id,
            "title": story.title,
    ...
    return {
        "epic": epic_id,
        "num_stories": len(plan.proposal.stories),
        "stories": stories_data,
        "notes": plan.proposal.notes,
        "validation_valid": plan.validation.valid,
        "validation_errors": plan.validation.errors,
        "self_audit": (
            {
                "status": plan.self_audit.status,
                "justification": plan.self_audit.justification,
                "suggestions": plan.self_audit.suggestions,
            }
            if plan.self_audit
            else None
        ),
...
def post_propose_tasks(us_id: str) -> dict:
    ...
    # T-AF008-US15-02 (2026-08-17): solo se puede proponer Tasks sobre
    # una User Story en `TO_PLAN` (AF-040; antes EN_DISEÑO) — evita
    # generar Tasks duplicadas o incoherentes sobre una Story ya desgranada
    # (antes, este endpoint no comprobaba en absoluto el estado real de la US).
    ...
    if us_state != "TO_PLAN":
        raise HTTPException(
            status_code=400,
            detail=(
                f"La User Story '{us_id}' no está en estado 'TO_PLAN' "
    ...
    # endpoint): el regex extraía `AF\d+` de `us_id` (p. ej. "US-AF999-03"
    # -> "AF999", SIN el guion que exige el formato real `AF-NNN`) — el
    ...
    # `us_id` real: "US-AF999-03" -> partes ["US", "AF999", "03"] -> epic
    # "AF-999".
    us_id_parts = us_id.split("-")
    if len(us_id_parts) >= 2 and us_id_parts[1].startswith("AF"):
        epic_id = "AF-" + us_id_parts[1][len("AF"):]
    else:
        epic_id = us_id
    ...
    # T-AF008-US15-02, criterio 4/5: en cuanto `propose-tasks` completa
    # con éxito (al menos una Task real escrita a disco), la User Story
    # transiciona automáticamente de `TO_PLAN` (AF-040; antes EN_DISEÑO)
    # al estado derivado `READY` — mismo fichero que ya se verificó arriba
    # en `TO_PLAN` antes de llegar aquí, así que la transición siempre
    # parte de ese estado real.
    if pipeline_result.approved_tasks:
        set_item_state(us_file_path, "READY")
...
@router.get("/backlog/us/{us_id}/report")
def get_us_closing_report(us_id: str) -> dict:
    """Informe de cierre real de la User Story `us_id` (T-AF036-US06-01):
    localiza el fichero real dentro de `07-informes/<us_id>/` por GLOB del
    directorio (NUNCA construyendo el nombre del fichero por string — el
    nombre real no siempre coincide con `<story_id>.md`, caso real
    confirmado) y devuelve su contenido completo.

    Casos explícitos (criterios de la Task/US):
    - US inexistente en el backlog -> 404 verbatim (un error real).
    - Directorio `07-informes/<us_id>/` inexistente o vacío -> 200 con
      `{"exists": false}` — resultado distinguible de un error real.
    - Informe presente -> 200 con `{"exists": true, "us_id", "path",
      "content"}` (contenido literal, saltos de línea preservados)."""
    project = get_active_project(state_dir=_STATE_DIR)
    if project is None:
        raise HTTPException(status_code=404, detail="No hay ningun proyecto activo.")

    graph = _load_active_backlog_graph(project)
    if us_id not in graph.items:
        raise HTTPException(
            status_code=404,
            detail=f"No existe ningun fichero de User Story con id '{us_id}'.",
        )

    reports_dir = Path(project.path) / "07-informes" / us_id
    candidates = sorted(reports_dir.glob("*.md")) if reports_dir.is_dir() else []
    if not candidates:
        return {"exists": False, "us_id": us_id}
...
def get_scripts() -> list[dict]:
    """Catálogo combinado de scripts del proyecto activo (T-AF018-US01-03):
    genéricos (`list_generic_scripts`, T-AF018-US01-01) + particulares
    (`discover_project_scripts`, T-AF001-US03-01), distinguibles por el
...
def post_script_run(script_id: str, request: RunScriptRequest | None = None) -> dict:
    ...
    T-AF018-US01-03: resuelve `script_id` contra AMBOS catálogos — si
    pertenece al catálogo genérico (T-AF018-US01-01) delega en
    `run_generic_script`, si no en `run_project_script` (T-AF001-US03-02).
    ...
    T-AF018-US02-04: para `backlog_status` (T-AF018-US02-02), además de la
    ...
    opcional de síntesis en prosa de T-AF018-US02-03, cuando Scribe/Ollama
    ...
    if script_id == "backlog_status" and result.success:
        # T-AF018-US02-04: el informe estructurado ya calculado (JSON en
        # stdout) se expone parseado para que el cliente lo presente con
        # formato, y la capa opcional de síntesis en prosa (T-AF018-US02-03)
