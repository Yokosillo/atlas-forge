"""Scribe (T-AF014-US01-01, AF-014 Local Tools): catálogo cerrado de
operaciones deterministas ejecutadas por un modelo local (Ollama), sin
tmux ni sesión interactiva — no es un `Agent` en el sentido de AF-005 (ver
`US-AF014-01`). Cada operación es una llamada HTTP síncrona e
independiente, sin estado retenido entre invocaciones.

Cliente HTTP: se usa `requests` (añadida a `04-src/pyproject.toml`, no
estaba declarada — decisión explícita de esta Task, ver T-AF000-01). Se
...
## Degradación explícita (T-AF014-US01-02, criterio de aceptación 5 de
US-AF014-01)
...
manual; el Dispatcher en US-AF008-03 de forma automática): Scribe es una
ayuda opcional para ahorrar tokens del agente principal (criterio 4 de
US-AF014-01), nunca una dependencia dura. Quien la invoque **debe**
...
resto de Atlas Forge. Ver `test_scribe.py`,
...
# Plantillas de prompt fijas por operación (T-AF014-US01-01, criterio de
# aceptación 3 de US-AF014-01: catálogo cerrado, no prompt libre).
...
def _unavailable_message(error: requests.RequestException, base_url: str, model: str) -> str:
    ...
    específico (T-AF014-US01-02, criterio de aceptación: "modelo no
...
def resumir_estado_backlog(
    ...
    JSON ya calculado por `build_backlog_report`/`atlas_forge backlog-status`
    (T-AF018-US02-02). Scribe NO vuelve a leer ni a parsear los ficheros de
    `02-backlog/`: recibe el JSON como única entrada y solo redacta sobre
    esos datos ya resueltos (criterios de aceptación 1 y 2 de T-AF018-US02-03).
    Capa opcional sobre el cálculo determinista — si el modelo local no está
    disponible, `atlas_forge backlog-status` sigue funcionando igual. Plantilla
...
def index_scripts(
    ...
    el modelo local configurado (T-AF014-US02-01, US-AF014-02).

    La entrada es la lista combinada de scripts de un proyecto — genéricos
    (`GenericScriptEntry`, T-AF018-US01-01) y particulares (`ScriptEntry`,
    T-AF001-US03-01) — serializados a texto plano por esta función (id,
