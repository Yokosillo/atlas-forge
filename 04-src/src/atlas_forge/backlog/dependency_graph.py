...
para una Epic (AF-026): construccion del grafo (US01-01), calculo de nivel
topologico (US02-01), agrupacion en hilos y deteccion de cruces (US02-02),
y recomendacion de reparto entre agentes (US03-01).

Reutiliza los datos ya extraidos por `atlas_forge.backlog.parser` — no
...
def group_threads_and_detect_crosses(
    ...
    # varias Tasks del mismo hilo destino (p. ej. T-AF022-US05-02 depende
    # de las 4 Tasks de T-AF022-US06-*), eso es UN cruce real "esta Task
    # depende del hilo destino", no uno por cada arista individual —
    # criterio de aceptación de la Epic AF-026 (corrección 2026-08-06,
    # auditoría de cierre de Fase 0.9: la versión sin deduplicar reportaba
    # 6 cruces sobre el caso real de AF-022 en vez de los 3 reales).
...
def persist_thread_analysis(
    ...
    Mismo patron que AF-025 (acciones transversales) y AF-022-US06-03
