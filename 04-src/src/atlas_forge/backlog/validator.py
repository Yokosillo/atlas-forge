...
# Validador LEGACY del formato Markdown antiguo (`## Estado:`), fuera del
# pipeline activo (AF-027 lo sustituyó por `validator_v2`). Solo lo usa su
# propio test. Grafía `TODO`/`IN_REVIEW` conviven aquí como compatibilidad
# documentada — el vocabulario canónico vive en `atlas_forge/core/state_machines.py`
# (guardia de estados literales, T-AF040-US01-05, lo excluye expresamente).
_VALID_STATUSES = {"TODO", "IN_PROGRESS", "REVIEW", "DONE", "IN_REVIEW", "READY", "TO_DEVELOP"}

_TITLE_PATTERNS = {
    "epic": re.compile(r"^# AF-\d{3} · .+$"),
    "user-story": re.compile(r"^# US-AF\d{3}-\d{2}[A-Z]? · .+$"),
    "task": re.compile(r"^# T-AF\d{3}-US\d{2}[A-Z]?-\d{2} · .+$"),
}
...
_EPIC_REF_PATTERN = re.compile(r"^\*\*Epic:\*\*\s+AF-\d{3}$")
_US_REF_PATTERN = re.compile(r"^\*\*User Story:\*\*\s+US-AF\d{3}-\d{2}[A-Z]?$")

_DEP_FIELD_RE = re.compile(
    r"\*\*(AF-\d{3}|US-AF\d{3}-\d{2}[A-Z]?|T-AF\d{3}-US\d{2}[A-Z]?-\d{2})\*\*"
)
_STATUS_PATTERN = re.compile(
    r"^## Estado\s*$|^## Estado:\s*(TODO|IN_PROGRESS|REVIEW|DONE|IN_REVIEW|READY|TO_DEVELOP)$"
)
...
def validate_backlog_content(content: str) -> ValidationResult:
    ...
    if file_type is None:
        errors.append(ValidationError(
            ...
            "IDs esperados: AF-NNN (Epic), US-AFNNN-nn (User Story), "
            "T-AFNNN-USnn-mm (Task)"
        ))
    ...
    if file_type in ("user-story", "task"):
        for i, line in enumerate(lines):
            ...
            errors.append(ValidationError(
                0,
                "Falta el campo obligatorio '**Epic:** AF-NNN'"
            ))
    if file_type == "task":
        for i, line in enumerate(lines):
            ...
            errors.append(ValidationError(
                0,
                "Falta el campo obligatorio '**User Story:** US-AFNNN-nn'"
            ))
