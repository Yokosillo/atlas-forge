"""Detalle de un item concreto del backlog (`T-AF020-US01-01`,
US-AF020-01 · "Ver el estado del backlog: Epics, User Stories y su
detalle").
...
`user-stories/` y `tasks/`) — viven en `02-backlog/epics/AF-xxx-*.md` y se
identifican por el prefijo `AF-xxx` de su nombre de fichero. La etiqueta
`item.epic` de cada `BacklogItem` es el texto libre tras `**Epic:**` (p.
ej. `"AF-020 · Gestión de Backlog (alcance v1)"`) — distintas Tasks/US de
la MISMA Epic real pueden traer sufijos distintos (verificado sobre el
backlog real de este proyecto: `AF-008` aparece con 8 variantes de
sufijo). Por eso el desglose de una Epic agrupa por el prefijo `AF-xxx`
...
_EPIC_LABEL_PREFIX_PATTERN = re.compile(r"^(AF-\d{3,})")
...
def _read_section(text: str, header_name: str) -> str | None:
    ...
    comportamiento. `T-AF018-US02-05` normalizará los encabezados de las
    ...
    Objetivo del resto del fichero (`AF-020-gestion-de-backlog.md` entre
...
def is_epic_item_id(item_id: str) -> bool:
    """True si `item_id` tiene la forma `AF-xxx` (identificador de Epic,
    p. ej. `AF-020`) en vez de `T-FBxxx-...`/`US-FBxxx-...` (Task/User
...
def build_epic_detail(backlog_path: str | Path, graph: BacklogGraph, epic_id: str) -> dict | None:
    """Detalle de la Epic `epic_id` (p. ej. `AF-020`): su objetivo (leído
    ...
    T-AF022-US13-05: si la propia Epic o alguna de sus User Stories
    ...
    if epic_file is not None:
        ...
        # T-AF036-US18-01: la Epic se versiona — `version` se lee del
        # frontmatter real (`version:` que escribe `create_epic`, default
        # "0.9"), con respaldo al formato legacy `## Version`/`## Fase` para
        # Epics antiguas que aún declaraban la fase en el cuerpo. La Epic ya
        # NO expone `fase`.
        epic_item = graph.items.get(epic_id)
        version = (
            getattr(epic_item, "version", None)
            or _read_section(text, "Version")
            or _read_section(text, "Fase")
        )
        if objetivo is None:
    ...
    drifted_ids = derive_graph_consolidation(graph)

    # T-AF036-US01-09: número de Tasks de cada User Story, contado sobre
    # el mismo `graph` ya cargado (sin llamada adicional al backend) —
    # se cruza por el campo real `item.user_story` de cada Task (el
    # valor del `user_story:` del frontmatter YAML, `T-AF008-US04-05`),
    # NUNCA por prefijo del propio id de la Task: confirmado en
    # `T-AF036-US01-04`/`T-AF008-US10-01` que esa convención de nombre
    ...
    result: dict = {
        ...
        "user_stories": [
            {
                "id": item.id,
                "state": drifted_ids.get(item.id, item.state),
                "priority": item.priority,
                # T-AF036-US19-01: título de la US (`title:` del frontmatter,
                # fallback al id si ausente).
                "title": item.title or item.id,
                # T-AF036-US13-03: fase del roadmap y fecha de última
    ...
    if epic_id in drifted_ids:
        result["state"] = drifted_ids[epic_id]
        result["drift"] = True
    return result
...
def build_item_detail(graph: BacklogGraph, item_id: str) -> dict | None:
    ...
    (T-AF008-US04-05: `dependencies` nunca fue la relación Task→US real —
    ...
    T-AF022-US13-05: si `item_id` es una User Story con drift inverso
    ...
    drifted_ids = (
        derive_graph_consolidation(graph) if item.kind == ITEM_KIND_USER_STORY else {}
    )
    derived_state = drifted_ids.get(item.id, item.state)
    is_drifted = derived_state != item.state
    ...
    result: dict = {
        ...
        "state": derived_state,
        "priority": item.priority,
        # T-AF036-US19-01: título del item (`title:` del frontmatter,
        # fallback al id si ausente).
        "title": item.title or item.id,
        # T-AF008-US11-02: dificultad estimada de la Task (`Crítica`/
        # `Alta`/`Media`/`Baja`, `T-AF008-US11-01`) — `None` para una User
        ...
        result["tasks"] = [
            {
                ...
                # T-AF036-US19-01: título de la Task (`title:` del
                # frontmatter, fallback al id si ausente).
                "title": task.title or task.id,
                # T-AF036-US13-03: fase y fecha de última transición de cada
                # Task anidada, para el encabezado de la web (`None` si el
                # fichero no los declara).
                "fase": task.fase,
