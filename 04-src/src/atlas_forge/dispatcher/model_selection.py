"""Selección de modelo basada en dificultad de Task (T-AF008-US12-01).
...
from atlas_forge.models_catalog import load_model_catalog, ModelEntry
from atlas_forge.system_preferences import get_difficulty_model_map
...
def get_models_for_difficulty(
    ...
    try:
        ...
        if project_path:
            catalog_path = Path(project_path) / ".atlas-forge" / "models.yml"
        else:
            # Fallback: buscar .atlas-forge en cwd
            catalog_path = Path.cwd() / ".atlas-forge" / "models.yml"
