...
desarrollador (T-AF008-US04-01, US-AF008-04 · "El Critic propone un plan
...
existente en el backlog (`US-AFNNN-nn`, ver `02-backlog/README.md`) —
...
AF-010 Capability Engine no existe todavía (ver US-AF008-04, "Contexto"):
...
`02-backlog/epics/AF-010-capability-engine.md` (Scripts deterministas /
...
- si no, pero menciona "Scribe" (el modelo local de AF-014), el paso se
...
sin un catálogo real de capacidades (AF-010), no hay forma fiable de
...
# T-AF008-US04-07: coincidencia por PALABRA COMPLETA (`\b...\b`), no
...
# `US-AF036-02`). `\b` no reconoce acentos como límite de palabra en
...
def task_file_story_prefix(story_id: str) -> str:
    ...
    (`AF020-US01`), aceptando tanto la forma canónica del backlog
    (`US-AF020-01`) como la ya normalizada (`AF020-US01`).

    - `US-AF020-01` -> `AF020-US01`
    - `AF020-US01`  -> `AF020-US01` (idempotente)

    Los nombres de fichero reales de Task son `T-AF020-US01-...md`
    (NUNCA `T-US-AF020-01-...md`), así que el glob `T-{story_id}-` que
...
def _task_correlative(task_path: Path) -> int:
    # T-AFNNN-USnn-mm-<slug>.md -> mm, para ordenar igual que aparecen en
    # el backlog (criterio de aceptación de la Task).
    match = re.match(r"T-AF\d+-US\d+-(\d+)-", task_path.stem)
...
def _read_task_state(text: str) -> str:
    ...
    (formato vigente, AF-027), con fallback a la sección Markdown antigua
    `## Estado` si el fichero no tiene frontmatter (Task legacy sin migrar
    — criterio 4 de T-AF008-US04-05: no romper ese caso si quedara
...
def _pending_task_files_for_story(story_id: str, tasks_dir: Path) -> list[Path]:
    ...
    for path in candidates:
        ...
        if _read_task_state(text) == "READY":
            pending.append(path)
    return pending
...
def build_job_plan_for_story(
    ...
    (p. ej. `"US-AF008-04"`): un `JobPlanStep` por cada Task en estado
    ...
    — no se despacha nada (T-AF008-US04-03)."""
