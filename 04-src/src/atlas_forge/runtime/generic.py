...
from atlas_forge.tmux import create_session, is_alive, kill_session, run_command, send_keys_literal
...
# `Runtime.type` (T-AF005-US01-03) — mismo patrón dict-dispatch ya usado
...
def _prompt_args_builder_by_type() -> dict[str, Callable[[str], list[str]]]:
    from atlas_forge.runtime.claude_code import build_prompt_args as claude_code_prompt_args
    from atlas_forge.runtime.codex import build_prompt_args as codex_prompt_args
    from atlas_forge.runtime.opencode import build_prompt_args as opencode_prompt_args
...
def sanitize_session_name_part(value: str) -> str:
    ...
    (`tmux attach -t <nombre>`, ver criterio 4 de `T-AF030-US01-01`). Se
    ...
    Pública (sin guion bajo) desde T-AF031-US02-02: `core/
...
def session_name_for(runtime: Runtime, agent: Any, project_path: str) -> str:
    ...
    multi-instancia, p. ej. Developer) — AF-030/US-AF030-01, sustituye el
    ...
    conservado por compatibilidad con tests de AF-004 anteriores a AF-005
...
class ParsedSessionName:
    ...
    un nombre de sesión tmux normalizado (AF-030), y su número de
...
def parse_session_name(name: str) -> ParsedSessionName | None:
    """Función inversa de `session_name_for` (AF-031/US-AF031-02): dado un
    ...
    workspace, p. ej. `PROD-006-atlas-forge`), así que no basta con
    ...
    1. Lista CERRADA de roles válidos conocidos — `atlas_forge.agents.list_roles()`,
       ...
       de la función: `runtime/generic.py` no puede importar `atlas_forge.agents`
       a nivel de módulo sin crear un ciclo, ya que `agents/developer.py`
       y `agents/arquitecto.py` ya importan `atlas_forge.runtime` para
    ...
    """
    from atlas_forge.agents import list_roles
...
def start_runtime(
    ...
    por defecto usa el servidor propio de Atlas Forge.

    ## Prompt inicial del agente (T-AF005-US01-03)
...
def extract_model_from_runtime(runtime: Runtime) -> str | None:
    ...
    (T-AF005-US05-01, `GET /agents` necesita reflejarlo).
