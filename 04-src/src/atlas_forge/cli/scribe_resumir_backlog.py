"""Subcomando `atlas_forge scribe resumir-backlog` (T-AF018-US02-03, US-AF018-02 ·
...
T-AF018-US02-02: lee por stdin el JSON que produce
`atlas_forge backlog-status --json`, lo pasa a `resumir_estado_backlog` (Scribe,
...
`atlas_forge backlog-status` — la síntesis es una capa añadida, no una dependencia
...
def run_resumir_backlog(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="atlas_forge scribe resumir-backlog",
        description="Redacta un resumen en prosa del estado del backlog "
        "a partir del JSON de `atlas_forge backlog-status --json`.",
    )
    ...
    if not raw:
        print(
            ...
            "'atlas_forge backlog-status --json <02-backlog/> | "
            "atlas_forge scribe resumir-backlog'.",
            file=sys.stderr,
        )
        return 2
    try:
    ...
    try:
        ...
        print(
            ...
            f"el estado del backlog sigue disponible con 'atlas_forge "
