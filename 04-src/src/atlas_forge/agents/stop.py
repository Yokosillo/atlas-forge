"""Detener un agente lanzado a propósito (T-AF016-US01-03): capacidad de
dominio que hoy no existía en ninguna interfaz, aunque su mecanismo de
bajo nivel (`stop_runtime`/`is_runtime_alive`, `atlas_forge.runtime.generic`) ya
estaba implementado desde AF-004, sin ningún consumidor hasta ahora.

T-AF024-US12-02: para Developer, "detener" pasó de pausar (`stopped`
...
from atlas_forge.agents.developer import DEVELOPER_ROLE
from atlas_forge.agents.lifecycle import mark_stopped
...
from atlas_forge.runtime.generic import stop_runtime
...
def stop_agent(
    ...
    T-AF016-US01-03) — y `agent` permanece en `session.agents`,
    reutilizable/consultable como una instancia pausada.

    Para `DEVELOPER_ROLE` (T-AF024-US12-02, decisión de producto): en vez
    ...
    `get_runtime_instance_for_agent` (T-AF002-US03-00); si no hay ninguno
