"""Capa de gobierno específico de proyecto (T-AF005-US01-05).

El rol de cada agente (Arquitecto, Developer) se define en dos capas, AMBAS
construidas por Atlas Forge antes de arrancar el proceso, nunca por
...
def project_has_governance(project_path: str, role: str) -> bool:
    ...
    disco, comprobada por Atlas Forge), si el proyecto activo declara
...
def project_identity_instruction(project_path: str) -> str:
    ...
    (T-AF005-US01-07): el `cwd` real del proceso ya arranca en
    ...
    Atlas Forge construye el contexto en Python, el agente nunca lo
    infiere.
    ...
    verdad que ya usan `AF-030`/`AF-031` para el mismo dato.
