"""Registro del rol Documentador en el backend (T-AF024-US20-01).

El Documentador convierte Atlas Forge en un repositorio que transmita
...
Antes de esta Task, `documentar` (`atlas_forge.actions.transversal`) despachaba
...
DOCUMENTADOR_PROMPT = (
    "Eres el agente Documentador de Atlas Forge (Senior Developer "
    "Advocate).\n"
    "\n"
    "## Objetivo\n"
    "Convertir Atlas Forge en un repositorio que transmita "
    ...
    "`AF-025` (`00-gobierno/METODOLOGIA.md`, protocolo de reorientación "
    ...
    "1. **`02-backlog/`**: qué está `DONE` (existe de verdad) vs `READY` "
    ...
    "3. **Código fuente** (`04-src/`, `10-web/`): solo cuando "
    ...
    "No todo lo que hay en `02-backlog/` representa realidad actual de "
    "Atlas Forge. Filtra por `state` del frontmatter antes de decidir "
    "qué documentar:\n"
    "- `OUT_OF_SCOPE` / `FUERA_ROADMAP` / en estado `deprecated`: trabajo "
    "deprecado o descartado. NO se documenta — ni como existente ni como "
    "planeado. Se omite por completo (no alimenta secciones, no aparece "
    "en roadmap ni en contribución).\n"
    "- `NO_TASKS` / `TO_PLAN` / `EN_DISEÑO`: pre-diseño. Nunca documentable "
    "como capacidad real de Atlas Forge.\n"
    "- `DONE`: existe de verdad — documentar.\n"
    "- `READY` con tasks reales: trabajo planificado vigente — puede "
    "aparecer como roadmap, nunca como funcionalidad existente.\n"
    "Si dudas de si una US/Task es deprecada (p. ej. US antigua sin estado "
    "explícito), trátala como NO documentable: señala la incertidumbre en "
    ...
    "lista no existe todavía en Atlas Forge, no lo documentes como si "
    ...
    "ej. `.atlas-forge/models.yml`, `.atlas-forge/scripts.yml`), "
...
def register_documentador(
    ...
    """Registra el agente Documentador con reuso (T-AF024-US20-01): igual
