from atlas_forge.agents import register_developer
from atlas_forge.agents.roles import get_register_fn_for_role, list_roles
...
def launch_agent(
    ...
    OpenCode y Claude Code (T-AF024-US11-13, 2026-08-17: Claude Code
    admite `--model` al arrancar, corrección de la premisa original de
    T-AF002-US01-01 que asumía lo contrario sin verificarlo). Cualquier
    rechazo se señala con `AgentLaunchError` (motivo explícito).

    Reutiliza `register_developer`/`register_arquitecto` (AF-005). Desde
    T-AF005-US01-04, no se comportan igual: `register_arquitecto` reutiliza
    ...
    internamente desde el proyecto activo persistido, AF-001): esta
    ...
    (la futura interfaz, T-AF002-US01-04) resolver el proyecto activo y
    ...
    if role == "developer":
        # T-AF005-US01-08 (2026-08-18): cada Developer es un slot
...
def launch_agent_with_initial_job(
    ...
    firma ni su retorno de 2-tupla — T-AF008-US06-01) y, si
    ...
    `atlas_forge/api/events.py`); quien invoque este flujo combinado desde la
