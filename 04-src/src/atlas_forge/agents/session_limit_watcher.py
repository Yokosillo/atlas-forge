...
reset (T-AF024-US21-01, US-AF024-21).

Mismo patrón arquitectónico que `dispatch_queue_worker.py`
(`DispatchQueueWorker`, T-AF008-US10-02): un hilo `daemon` DENTRO del
propio proceso `atlas-forge-api`, no un script externo — necesita
...
`tmux send-keys` separada. `atlas_forge.tmux.manager.run_command` ya cumple
...
from atlas_forge.agents.lifecycle import InvalidAgentTransitionError, clear_session_limit, mark_limited
from atlas_forge.agents.session_limit import detect_session_limit_block, should_ping_now
...
from atlas_forge.tmux.manager import DEFAULT_SOCKET_NAME, capture_pane_lines, run_command
