"""Registro consultable de `RuntimeInstance` por `agent.id` (T-FB002-US03-00,
FB-002 Dashboard): cierra el gap de que `Agent` (FB-005) no guarda
ninguna referencia a su `RuntimeInstance` (FB-004) — hasta ahora ese dato
...
(T-FB002-US03-01) necesita recuperar el `RuntimeInstance` del agente
...
dominio de FB-005 (agentes conversacionales) a un detalle de FB-004
(runtimes/tmux) — mismo criterio ya aplicado en T-FB008-US03-02 para no
...
`RuntimeInstance`, un concepto de FB-004, y no depende de nada de
...
from brain.runtime.generic import RuntimeInstance
...
def register_runtime_instance_for_agent(
    ...
    asociación anterior (mismo criterio que `save_active_project`, FB-001:
