...
# 2026-08-18, unificación de grafía (T-FB040-US01-01): `TO_DO` es el valor
# "pendiente de empezar" de los tres tipos. Epic queda fuera por completo
# de `set_item_state` (ver docstring de módulo); este conjunto se aplica
# a US/Task.
...
# faltaba `FUERA_ROADMAP`, ya soportado por el validador). `EN_DESARROLLO` es el
# valor nuevo de esta Task: "lista para desarrollo" vive en el propio
# `state`, no solo en `dispatch_queue.json`.
#
# 2026-08-17, "PIPELINE OPERATIVO Y RECONCILIACIÓN": `NO_TASKS`/`EN_DISEÑO`
...
VALID_STATES = ("TO_DO", "NO_TASKS", "EN_DISEÑO", "EN_DESARROLLO", "IN_PROGRESS", "REVIEW", "DONE", "FUERA_ROADMAP")
