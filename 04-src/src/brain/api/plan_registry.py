"""Registro de `JobPlan` por identificador HTTP (T-FB016-US01-05).

`JobPlan` (dominio, US-FB008-04) no tiene `id` — no lo necesita ahí: el
...
(`brain/api/`) y no en `brain/dispatcher/` junto al resto de registros de
dominio (`job_history_registry`, `job_count_registry`).

## Lock por plan (T-FB016-US01-08)
...
(T-FB016-US01-01, criterio de aceptación explícito de esta Task)."""
...
def list_plans() -> list[tuple[str, JobPlan]]:
    ...
    (T-FB016-US01-14) — `(plan_id, JobPlan)` en orden de registro. Ninguno
...
def get_plan_lock(plan_id: str) -> threading.Lock:
    ...
    mismo objeto `Lock` (T-FB016-US01-08, ver docstring de módulo)."""
