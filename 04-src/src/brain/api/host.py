"""Resolución del host en el que escucha el servidor (T-FB016-US01-01).

## Por qué nunca `0.0.0.0` por defecto en producción

FB-015 (T-FB015-US01-01) verificó en esta misma VM que escuchar en todas
...
pública. La Epic FB-016 fija explícitamente "sin autenticación propia — el
perímetro de seguridad es la pertenencia a la red Tailscale" (ver
`02-backlog/epics/FB-016-api-backend.md`, "Alcance v1"): si el proceso
...
Por eso el host por defecto de `brain-api` se resuelve dinámicamente a la
...
def resolve_tailscale_host() -> str:
    ...
    T-FB015-US01-01). Lanza `TailscaleHostUnavailableError` con el motivo
