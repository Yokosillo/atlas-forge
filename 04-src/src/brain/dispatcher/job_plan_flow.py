"""Ciclo completo de dominio de un `JobPlan` (T-FB008-US04-04,
US-FB008-04): construir → presentar/aprobar → despachar → consultar
progreso, encadenado en una única función, sin pasar por ninguna interfaz
(TUI/API) — esa exposición es responsabilidad de T-FB016-US01-05 (endpoints/
WebSocket) y T-FB017-US01-04 (pantalla en la app), ver la corrección de
...
tal como haría cualquier llamador real (futura ruta de FB-016).
...
from brain.dispatcher.job_plan_dispatch import dispatch_plan
from brain.models import DevelopmentSession, JobPlan
...
def run_job_plan(
    ...
    depender del backlog real (T-FB008-US04-04).
    ...
    aceptación de T-FB008-US04-02/03, reconfirmado aquí en el flujo
