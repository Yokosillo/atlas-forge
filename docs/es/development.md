...
Guía para nuevos desarrolladores que quieren construir, probar, depurar y extender Atlas Forge.
...
No hay paso de compilación: es un paquete Python puro. `-e` enlaza el paquete al código fuente (los cambios en `04-src/src` se reflejan al instante). Los entrypoints `atlas_forge` y `atlas-forge-api` quedan disponibles en `.venv/bin/`.
...
- **Backend**: arranca con `atlas-forge-api --host 127.0.0.1` para desarrollo local y prueba contra `http://127.0.0.1:8000`. Los errores HTTP llevan el `detail` del dominio.
- **Web**: servida en `/ui/` del mismo backend; abre el inspector del navegador (consola + red).
- **Agentes/runtimes**: `GET /agents/{id}/pane` devuelve el contenido textual del pane de tmux del agente. También puedes `tmux -L atlas-forge attach` en el host.
...
- **Dominio** (`atlas_forge/`): sin dependencia de interfaz. Los clientes (la web) consumen solo la API (`atlas_forge/api/routes.py`).
...
1. Crea `04-src/src/atlas_forge/agents/<role>.py`:
...
Ejemplo real a seguir: `04-src/src/atlas_forge/agents/tester.py`.

## Cómo añadir un runtime

1. Crea `04-src/src/atlas_forge/runtime/<runtime>.py` con `register_<runtime>_runtime(runtime_id=...) → Runtime` (comando, args, tipo) y `build_prompt_args(prompt) -> list[str]` (cómo se pasa el prompt a la CLI).
...
Ejemplo real: `04-src/src/atlas_forge/runtime/opencode.py`.

## Cómo añadir una herramienta / acción

Para una **acción transversal** (pestaña Acciones de la web):
1. Añade la acción a `atlas_forge/actions/transversal.py` (`ACCIONES_DISPONIBLES` + una función `_dispatch_<action>_action`).
...
1. Añade la entrada a `GENERIC_SCRIPTS` en `atlas_forge/workspace/generic_scripts.py`.
...
2. Añade la ruta en `04-src/src/atlas_forge/api/routes.py` (devolviendo los objetos ya serializados del dominio; errores como `HTTPException` con el `detail` del dominio).
