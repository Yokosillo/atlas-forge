...
Atlas Forge es el sistema operativo de una fábrica de desarrollo basada en IA. Coordina proyectos, agentes, runtimes, Jobs y pipelines de trabajo, manteniendo vivo el contexto operativo a lo largo de todo el ciclo de desarrollo.
...
Atlas Forge centraliza ese flujo: el proyecto activo, la sesión viva, los agentes lanzados, el historial de Jobs, los scripts y el estado del backlog viven en un único proceso (`atlas-forge-api`) al que se conecta cualquier interfaz.

## Qué es realmente Atlas Forge

Atlas Forge es una **capa de coordinación entre el trabajo definido y los agentes que lo ejecutan**. Su propia declaración de visión es la definición más clara disponible: *"Atlas Forge automatiza la ejecución del trabajo, no las decisiones sobre qué trabajo hacer."*
...
**En qué se diferencia de un agente de codificación:** Claude Code hace el trabajo pero no sabe qué trabajo existe, no persiste entre sesiones por sí solo, no se coordina con otros agentes y no valida su propia salida. Atlas Forge es la capa que necesita un agente de codificación para convertirse en una *fábrica* en lugar de una herramienta.
...
|---|---|
| **Coordinación sobre ejecución** | Atlas Forge coordina; los agentes ejecutan con sus propios runtimes y modelos. El sistema no genera código directamente. |
...
- Código: [`04-src/`](https://github.com/factoria-software/atlas-forge/tree/main/04-src) — el paquete Python `atlas_forge`.
- Backlog canónico: [`02-backlog/`](https://github.com/factoria-software/atlas-forge/tree/main/02-backlog).