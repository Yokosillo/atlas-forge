#!/usr/bin/env bash
# watch_critic.sh — crítico -> worker
# Vigila critic_output.txt; al detectar una escritura, extrae el bloque
...
WORKER_SESSION="claude-factory-brain"
...
log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [watch_critic] $*" >> "$LOG_FILE"
}

log "arrancado (pid $$), vigilando $CRITIC_OUTPUT, sesión destino: $WORKER_SESSION"
...
if [ ! -f "$CRITIC_OUTPUT" ]; then
    log "AVISO: $CRITIC_OUTPUT no existe todavía al arrancar — inotifywait fallará hasta que se cree"
fi
...
inotifywait -m -e close_write -e moved_to -e create --format '%f' "$STATE_DIR" 2>>"$LOG_FILE" | while read -r changed_file; do
    if [ "$changed_file" != "critic_output.txt" ]; then
        continue
    fi
    log "evento detectado en $CRITIC_OUTPUT"

    NEXT_PROMPT=$(awk '/SIGUIENTE_PROMPT_PARA_WORKER:/{flag=1; next} flag' "$CRITIC_OUTPUT")
