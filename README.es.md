...
Atlas Forge centraliza ese flujo:
...
Atlas Forge es una **capa de coordinación entre el trabajo definido y los agentes que lo ejecutan** — no otro gestor de proyectos, y no otro agente de codificación. El backlog (Epic → User Story → Task) es el lenguaje usado para describir el trabajo; la sesión de desarrollo persistente, los Jobs y los pipelines son el mecanismo usado para ejecutarlo y verificarlo.
...
- **vs. Claude Code/Codex/OpenCode**: esos ejecutan trabajo pero no saben qué trabajo existe, no persisten entre sesiones y no validan su propia salida. Atlas Forge es la capa que convierte un agente de codificación en una fábrica.
...
Ver [Qué es realmente Atlas Forge](docs/es/index.md#qu-es-realmente-atlas-forge) en la documentación completa para el panorama completo, incluido el posicionamiento de mercado.

## Qué lo diferencia

- **Coordinación sobre ejecución**: Atlas Forge decide *quién hace qué y cuándo*; los agentes ejecutan con sus propios runtimes y modelos.
